...
import { createComissao, updateMembros } from "@/app/admin/_actions/comissoes"
...
export function ComissaoForm({ slug, vereadores, materias, onSuccess }: ComissaoFormProps) {
  ...
  const onSubmit = async (data: ComissaoInputs) => {
    setError(null)
    startTransition(async () => {
      // 1. Criar Comissão
      const result = await createComissao(slug, {
        nome: data.nome,
        tipo: data.tipo,
      ...
      if (!result.success || !result.id) {
        setError(result.error || "Erro ao criar comissão")
        return
      }

      // 2. Salvar Membros
      const membrosResult = await updateMembros(slug, result.id, data.membros)
      
      if (!membrosResult.success) {
        setError("Comissão criada, mas erro ao salvar membros: " + membrosResult.error)
        return
      }
